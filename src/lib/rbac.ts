...
export async function requireVerifiedEmail(
  ...
  const permitted = await enforcePermission(auth.user.id, options.permission);
  if (permitted instanceof NextResponse) return permitted;
  return auth;
}
...
async function enforcePermission(
  ...
  if (!perms.has(permission)) return FORBIDDEN;
  return true;
}
...
// Auth + optional permission + write-verification in one call.
export async function requirePermission(
  ...
  const permitted = await enforcePermission(auth.user.id, permission);
  if (permitted instanceof NextResponse) return permitted;
  return auth;
}
