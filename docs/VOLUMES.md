...
| V6 | Hardening program: seed prod-guard (refuses NODE_ENV=production without explicit opt-in) · `/api/health/performance` admin-gated + raw error text removed · `/api` index generic failure message · timing-safe scheduler key check · i18n parity test (caught + fixed hi dash gaps + hi onboarding junk keys) · guard-chain parity test (91 routes, 16 allowlisted) · CSP Report-Only→enforce (smoke 13/13) | `f29aa41` |
| V3 | Decision: middleware→proxy migration DEFERRED (codemod no-op on withAuth shape; next-intl 4.13 proxy-compat risk per Next codemod source; middleware fully works in 16.3). Trigger: Next drops middleware support or libs document proxy compat | `bd0928b` |
| V13 | SCIM Groups PATCH (RFC 7644 PatchOp: displayName/members add-remove-replace, 7 contract tests) + retention sweeps in scheduler tick (terminal scans cascade violations; audit/auth tables pruned; env-tuned windows; 3 tests) · `.env.example` now tracked (was gitignored, secrets scanned clean) | `02245f5` |
| V8 | Guided manual testing: 15-check bilingual catalog (keyboard/SR/alt/captions/zoom…), ManualCheck model (db push, CI workflow), per-project GET+PUT API (MANAGE_VIOLATIONS, audit event), project-page tab, 4 contract tests · BONUS FIX: GET /api/projects/[id] never existed — every detail page 404'd; added + 3 tests · browser-verified, 400 tests green | `de4340a` |
| V8 | Full manual QA pass (all routes + interactive flows): 28 missing i18n keys fixed (team components pointed at nonexistent keys — inviteMember/removeMemberTitle/changeRoleAria/etc — now reuse existing keys; 24 audit action labels added en+hi; audit.export + team.roleAria added) · legal links fixed (cookie banner + settings pointed at raw-markdown /api/legal/* → /privacy + /terms) · Redis dead-client guard (recreate singleton stuck in 'end' status) · new `scripts/i18n-audit.mjs` + `scripts/audit-actions-diff.mjs` regression guards · verified lint 0, tsc 0, vitest 403/403, browser re-verified | `c1bbac6` |
| V2 | Design de-slop (user: "AI generated lag raha hai"): fake testimonials wall (invented names/quotes) replaced with honest How-It-Works 3-step workflow · fake trust stats (500+/2M+/50K+/99.9%) replaced with capability strip · dead Watch-Demo modal (missing /demo.mp4) removed → "See how it works" scroll · dead footer links (#) wired to real routes (/pricing, /api/docs, /privacy, /terms, repo) + false SOC 2 badge removed · false SOC 2 claims removed (landing card/FAQ/privacy page; program is Year-1 roadmap per CERTIFICATION_PROGRAM) · dashboard honesty: fake "12% from last week" → real week-over-week delta, dead trend range selector wired (7/30/90d), dead header search → command-palette trigger, notification action IDs fixed to AUDIT_ACTIONS · severity/risk colors hardcoded hex → design tokens · brand assets recolored blue→coral · BONUS FIX: `/_next/static` immutable Cache-Control pinned stale Turbopack chunks in dev (every edit looked broken) → prod-only · verified lint 0, tsc 0, vitest 403/403 | `79331ef` |
| V7 | Dedicated scan-worker split: `scripts/scan-worker.ts` (`npm run worker`, SIGTERM/SIGINT graceful shutdown) + compose `worker` service (profile-gated, `--scale worker=N`, 4G mem limit) + `SCAN_WORKER_CONCURRENCY` (default 3, validated 1–32) + `DISABLE_BACKGROUND_WORKERS` split gate in instrumentation + `docs/devops/SCALING.md` capacity math · BONUS FIX: runner-stage Dockerfile now COPYs src/scripts/tsconfig (tsx needs TS sources + @/* paths; `.dockerignore` tsconfig exclusion lifted with rationale) · verified lint 0, tsc 0, vitest 403/403, compose config valid | `736b573` |
...
| 22 | Zero-Budget AI Mesh & WCAG 2.2 — OpenRouter Free + NVIDIA NIM auto-failover, $0 cost accounting, WCAG 2.2 criteria & templates | AI model mesh, free tier pricing, WCAG 2.2 rules | ✅ Done (commit 758bdb0) |
