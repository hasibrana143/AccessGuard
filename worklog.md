...
Task ID: 1
Agent: Main
Task: Remove demo projects and use real projects for testing

Work Log:
- Reviewed project structure and identified that API routes already use real database operations via Prisma
- Found seed.ts creates demo organization, demo user, demo projects, and demo violations
- Updated seed.ts to only create WCAG rules (reference data) and default organization - no demo projects/violations
- Updated API routes to use 'default-org' instead of 'demo-org'
- Updated frontend hooks (useProjects, useViolationStats) to use 'default-org'
- Cleared database and re-seeded with new seed script
- Updated DashboardView, ProjectsView, and ScansView to accept user prop and use user's organization slug
- Updated main App component to pass user prop to views

Stage Summary:
- Demo projects and violations removed from seed data
- Database cleared and re-seeded with minimal data (WCAG rules + default organization)
- Frontend now uses logged-in user's organization slug for API calls
- Users can register and create their own real projects
- The accessibility scanner performs real WCAG checks on actual URLs
